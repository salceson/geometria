...
from search_structures import KirkPatrickTriangles, BruteTriangles
from triangles import *

__author__ = 'Michał Ciołczyk, Michał Janczykowski'


def triangulate(points_list, search_struct_name='kirkpatrick', visualization=None):
    ...
    start_triangle = Triangle([Edge(p1, p2), None], [Edge(p2, p3), None], [Edge(p3, p1), None], '0.5')
    search_struct = KirkPatrickTriangles([start_triangle]) if search_struct_name == 'kirkpatrick' else BruteTriangles(
        [start_triangle])
    ...
    for point in points_list:
        triangle = search_struct.find(point)
        if triangle.is_inside(point):
            [ne1, nt1], [ne2, nt2], [ne3, nt3] = search_struct.delete(triangle)
            t1 = Triangle(
                [ne1, nt1],
                [Edge(ne1.p2, point), None],
                [Edge(point, ne1.p1), None]
            )
            t2 = Triangle(
                [ne2, nt2],
                [Edge(ne2.p2, point), None],
                [Edge(point, ne2.p1), t1]
            )
            t3 = Triangle(
                [ne3, nt3],
                [Edge(ne3.p2, point), t1],
                [Edge(point, ne3.p1), t2]
            )
            t2.n2[1] = t3
            t1.n2[1] = t2
            t1.n3[1] = t3
            ...
            if point in ne1:
                ...
                nt = nt3
            if nt:
                (pr, pi, pj, pk, pl) = (point, ne.p2, ne.p1, triangle.opposite_point(ne), nt.opposite_point(ne))
                ...
                (pr, pi, pj, pk) = (point, ne.p2, ne.p1, triangle.opposite_point(ne))
                search_struct.delete(triangle)
                e_kj = Edge(pk, pj)
                n_kj = triangle.get_neighbor(e_kj)
                e_ik = Edge(pi, pk)
        ...
        if visualization:
            ...
            for t in search_struct.triangles:
                if len([p for p in additional_nodes if p in t.get_points()]) > 0:
                    t.color = '0.5'
                visualization.add_figure(t)
            visualization.add_all_figures(search_struct.triangles)
            visualization.add_figure(Point(point.x, point.y, 'b'))
    ...
    search_struct.triangles[:] = [t for t in search_struct.triangles if
                                  len([point for point in additional_nodes if point in t.get_points()]) == 0]
